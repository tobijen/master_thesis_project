...
def preprocess_request_dict(request_dict):
    ...
    for cand in request_dict["candidates"]:
        query_passage.append((query, cand["doc"]))
...
def draw_confusion_matrix(gts, preds, qrel, model_name):
    ...
    ax.set_xlabel("LLM Label", fontsize=14)
    ax.set_ylabel("Human Label", fontsize=14)
    plt.savefig(f"conf_matrix/{qrel}-{os.path.basename(model_name)}.png")




def create_input_dicts(query_json, candidates):
    input_dicts = []

    for query_obj in query_json:
        # SINGLE
        if query_obj.get("single"):
            for single in query_obj["single"]:
                query_text = single["keyword"]
                query_id = single["id"]

                input_dict = {
                    "query": {"text": query_text, "qid": query_id},
                    "candidates": []
                }
                ...
                for cand in candidates:
                    ...
                    input_dict["candidates"].append(candidate_entry)

                input_dicts.append(input_dict)

        # COMBINATION
        if query_obj.get("combination"):
            for comb in query_obj["combination"]:
                query_list = comb["keywords"]
                query_id = comb["id"]
